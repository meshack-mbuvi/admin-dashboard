...
import { Controller, ControllerFieldState, ControllerRenderProps, DefaultValues, FieldValues, FormProvider, RegisterOptions, SubmitErrorHandler, SubmitHandler, UseFormStateReturn, useForm, useFormContext } from "react-hook-form";
...
function Form<T extends FieldValues>(props: PropsWithChildren<FormProps<T>>){
    ...
    return <FormProvider {...methods}>
        <form onSubmit={methods.handleSubmit(onSubmit, onValidationError)} className="flex flex-col gap-6">
            {children}
        </form>
    </FormProvider>
}
...
export function FormControl(props: FormControlProps) {
    ...
    return <div className="flex flex-col gap-1">
        {(label || description) && <div className="flex flex-col">
            {label && <Label htmlFor={name} className={clsx("text-sm", "flex", "justify-between", "items-center", "mb-1", {
                "text-red": hasErrors, 
            ...
            {description && <div className="text-gray-3 text-sm">{description}</div>}
        </div>}
        {children}
        {error?.message && <div className="text-red text-sm">{error.message as string}</div>}
    </div>
}
...
export interface BaseFormInputProps<T> extends Pick<FormControlProps, 'name' | 'label' | 'description' | 'helperText'> {
    onChange?: (value: T) => void;
    defaultValue?: T;
    validate?: Pick<RegisterOptions, 'validate' | 'min' | 'max' | 'maxLength' | 'minLength' | 'required'>;
    render: (params: ControllerRenderParams, onChange?: (value: T) => void) => React.ReactElement;
}
