...
export default function Checkbox(props: CheckboxProps) {
  ...
  return (
    <Component
      className={cn(
        "inline-flex items-center relative py-1",
    ...
    >
      <input
        ...
        disabled={disabled}
        className={cn("opacity-0 absolute", { "cursor-pointer": !disabled })}
      />
      <span
        className={cn("w-6 h-6 border rounded p-1", {
      ...
      >
        {checked && (
          <Check
            className={cn({ "text-white": !disabled, "text-gray-4": disabled })}
          />
        )}
      </span>
