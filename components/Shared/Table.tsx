import type { FiltersColumn, HeaderGroup, Row, Table } from "@tanstack/react-table"
...
declare module '@tanstack/table-core' {
    interface ColumnMeta<TData extends unknown, TValue> {
        filterComponent: (setFilter: FiltersColumn<TData>["setFilterValue"], filterValue: unknown) => any;
    }
}
...
export default function Table<T>(props: TableProps<T>) {
  ...
  return (
    <div className={clsx("w-full overflow-x-auto min-h-[300px]", isLoading && "opacity-70")}>
                  ...
                  <span className={clsx("w-full", header.column.getCanFilter() && "inline-flex justify-between")}>
                    ...
                    {header.column.getCanFilter() && <>
                      {header.column.columnDef?.meta?.filterComponent && 
                        header.column.columnDef?.meta?.filterComponent(header.column.setFilterValue, header.column.getFilterValue())}
                    </>}
                  </span>
                </th>
              ))}
          ...
          {tableConfig.getRowModel().rows.map((row: Row<T>) => (
            <tr key={row.id} className="h-14">
              {row.getVisibleCells().map((cell: any) => (
                <td key={cell.id} className="pr-4 whitespace-nowrap overflow-x-hidden text-ellipsis" style={{width: cell.column.getSize()}}>
      ...
      {tableConfig.getRowModel().rows.length === 0 && <div className="text-center w-full text-gray-4 mt-36">{noDataMessage || "None found"}</div>}
    </div>
  )
}
